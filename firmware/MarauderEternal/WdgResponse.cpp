...
bool copyReason(const char* start, const char* end, char* output,
                size_t outputSize) {
  ...
  while (written > 0 &&
         (output[written - 1] == ' ' || output[written - 1] == '"'))
    written--;
  output[written] = '\0';
  return written > 0;
}

bool extractJsonValue(const char* body, const char* key, char* output,
                      size_t outputSize) {
  char quotedKey[16];
...
bool extractWdgErrorReason(const char* response, char* output,
                           size_t outputSize) {
  ...
  for (const char* key : keys) {
    if (extractJsonValue(body, key, output, outputSize)) {
      if (containsIgnoreCase(output, "duplicate"))
        copyReason("Duplicate upload", "Duplicate upload" + 16, output,
                   outputSize);
      else if (containsIgnoreCase(output, "already") &&
               containsIgnoreCase(output, "upload"))
        copyReason("Already uploaded", "Already uploaded" + 16, output,
                   outputSize);
      return true;
    }
  }
